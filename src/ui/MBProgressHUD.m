...
- (void)launchExecution;
- (void)cleanUp;
- (void)updateOnIndicatorTypeChange;
...
- (void)setMode:(MBProgressHUDMode)newMode {
    ...
    [self updateOnIndicatorTypeChange];
}
...
- (void)setDeterminateStyle:(MBDeterminateStyle)newStyle {
    // Dont change style if it wasn't actually changed to prevent flickering
    if (determinateStyle && (determinateStyle == newStyle)) {
        return;
    }

    determinateStyle = newStyle;

   [self updateOnIndicatorTypeChange];
}

- (MBDeterminateStyle)determinateStyle {
	return determinateStyle;
}
...
- (void)updateProgress {
    if (self.determinateStyle == MBDeterminateStyleRound) {
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        if (self.determinateStyle == MBDeterminateStyleRound) {
          ...
          self.indicator = [[[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleDefault] autorelease];
        }
    }
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.determinateStyle = MBDeterminateStyleRound;
        self.labelText = nil;
